...
from STAR_outliers_plotting_library import plot_x_and_W
...
def estimate_tukey_params(W, A2 = None):
...
def estimate_tukey_mixture(W, name, stable = True):

    # parameter initialization
    A1, A2 = np.percentile(W, 5), np.percentile(W, 95)
    B1, B2 = np.std(W)/2, np.std(W)/2
    g1, g2, h1, h2 = 1E-6, 1E-6, 1E-6, 1E-6
    pi1, pi2, N, cutoff = 0.5, 0.5, 100000, 0.5
    ...
    for i in range(1000):
        ...
        # E step part2
        L = [T1.pdf(W, stable) +1E-16, T2.pdf(W, stable) + 1E-16]
        P = [L[z]/np.sum(L, axis = 0) for z in range(2)]
        CDF = np.cumsum(P, axis = 0)
        rand = np.random.rand(len(P[0])).reshape(-1,1)
        Z = np.argmax(rand <= CDF.T, axis = 1)

        # M step
        A1, B1, g1, h1, status1 = estimate_tukey_params(W[Z == 0], A2)
        A2, B2, g2, h2, status2 = estimate_tukey_params(W[Z == 1], A1)
        if (status1 == False or status2 == False) and stable == True:
            return(estimate_tukey_mixture(W, name, stable = False))
        ...
        if LL_seq[-1] > LL_seq[LL_argmax]:
            ...
            counter += 1
        if counter == 30:
            return(param_seq[LL_argmax])
                      
    return((A1, B1, g1, h1, pi1), (A2, B2, g2, h2, pi2))
...
def get_outlier_fit(x, x_spiked, name, pcutoff, spike_vals, prefix):
    ...
    if is_multimodal or is_low_count:
        x_outliers, area_overlap = fit_tukey(x, x_body, mirrored_data, None, n_bins,
                                             dist_type, name, pcutoff, spike_vals, prefix)
    elif is_monotonic:
        x_outliers, area_overlap = fit_tukey(x, x_body, mirrored_data, status, n_bins,
                                             dist_type, name, pcutoff, spike_vals, prefix)
    else:
        ...
        for x_side, side in [(x_left, "_left"), (x_right, "_right")]:
            x_outliers, area_overlap = fit_tukey(x_side, x_body, mirrored_data, side, n_bins,
                                                 dist_type, (name + side), pcutoff, spike_vals, prefix)
...
def fit_tukey(x, x_body, mirrored_data, side, n_bins,
              dist_type, name, pcutoff, spike_vals, prefix):
    ...
    if is_low_count:
        ...
        if test_multimodality(get_body(W), n_bins):
            theta1, theta2 = estimate_tukey_mixture(W, name)
            A1, B1, g1, h1, pi1 = theta1
            A2, B2, g2, h2, pi2 = theta2
            z1 = np.random.normal(0, 1, int(pi1*100000))
            z2 = np.random.normal(0, 1, int(pi2*100000))
            TGH1  = A1 + B1*(1/(g1 + 1E-10))*(np.exp((g1 + 1E-10)*z1)-1)*np.exp(h1*(z1**2)/2)
            TGH2  = A2 + B2*(1/(g2 + 1E-10))*(np.exp((g2 + 1E-10)*z2)-1)*np.exp(h2*(z2**2)/2)
    ...
    if (is_monotonic or is_none) and (side == '_left' or side == "increasing"):
        ...
        x_outliers = x_outliers[x_outliers > np.median(x)]
        x = x[x >= np.median(x)]
    area_overlap = plot_x_and_W(x, x_outliers, spike_vals, W, fitted_TGH, name, prefix, cutoff, n_bins)
